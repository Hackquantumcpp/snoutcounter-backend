...
# Rasmussen is allowed for cycle<=2014
source("banned_pollsters.R")

setwd("../")

ratings <- read_tsv("ratings/pollster-ratings-fte-2014.tsv") %>% janitor::clean_names()

year <- "2014"

filepath <- paste0("data/historical_gb/generic_ballot_polls_", year, ".csv")
...
if (year == "2014") {
  labor_day <- ymd("2014-09-01")
}
if (year == "2016") {
  labor_day <- ymd("2016-09-05")
}
...
poll_avg <- function(data_frame, date) {
  ...
  ### Quality weights
  df <- df %>%
    left_join(ratings, join_by(pollster)) %>%
    mutate(
...
if (year == "2016") {
  polls <- polls %>% mutate(end_date = mdy(end_date), start_date = mdy(start_date)) %>%
    group_by(pollster, sponsors, start_date, end_date) %>%
    mutate(poll_id = cur_group_id()) %>%
    ungroup()
}
...
pop_a <- ranef(fit)$population[2, 1] # [2, 1] for 2014, [1, 1] for 2016
...
ggplot(
  ...
  labs(
    x = "Date",
    y = "Rep-Dem Spread %",
    title = paste("Generic Ballot Spread", year)
  )
